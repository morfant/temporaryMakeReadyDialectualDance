...
ChannelEQ {
    ...
    var <target, <numChannels, <server, <bus, <>inBus;
    var <synth, synthdef;
    ...
    *new { |numChannels, server, inBus, bus, target|
        var eq;
        eq = super.new.route(numChannels, server, inBus, bus, target)
            .init
            .initGUI
            .play
            .addCmdFunc;
    ...
    *newBare { |frdb|
    ...
    route { |argNumChannels, argServer, argInBus, argBus, argTarget|
        ...
        if (argTarget.notNil
            ...
            if(argInBus.isNil) {
                ...
                inBus = argInBus.asBus;
            };
        };
    }

    init { |argFrdb, argInBus|
        frdb = argFrdb ? [[100,0,1], [250,0,1], [1000,0,1], [3500,0,1], [6000,0,1]];

        synthdef = SynthDef("param_beq", { | in = 100, out = 0, gate = 1, fadeTime = 0.05, doneAction = 2|
            var frdb, input, env;
        ...
        cmdPeriodFunc = {
            server.waitForBoot {
                SystemClock.sched(0.5, {
                    // gui.bypassButtonState.postln;
                if (gui.bypassButtonState == 1, {
                    if (target.notNil) {
                        // using MixerChannel
                        synth = target.playfx(\param_beq, [\eq_controls, this.toControl]);
                    } {
                        // using raw Bus
                        synth = Synth.tail(server, \param_beq, [\in, inBus, \out, bus, \eq_controls, this.toControl]);
                    };
...
ChannelEQGUI {
    ...
    var uvw, font;
    var frpresets;
    var bypassButton, <bypassButtonState = 1;
    var selected;
    var tvw, tvwViews;
    ...
    *new { |channelEQ|
    ...
    onSelect {|argPresetName|
        var idx = puMenu.value;
        var selectedIdx, i = 0;

        // var selectedPresetName;
        // if (argPresetName.notNil, {selectedPresetName = argPresetName});

        // idx.postcs; // 0 ~ 4
        // puMenu.items.size.postcs; //5
        idx.switch(
            puMenu.items.size - 1, { },
            puMenu.items.size - 2, { puMenu.value = puMenu.items.size - 1; },
            {
                ...
                if (argPresetName.isNil,
                    {
                        channelEQ.frdb = frpresets[(puMenu.value * 2) + 1].deepCopy;
                    },
                    {
                        // var frpresetsDict = frpresets.asDict;
                        // frpresetsDict.postcs;

                        if (frpresets.clump(2).detectIndex({|item| item[0] == argPresetName}).isNil, {
                            SCAlert("Can't find  '%' in preset list.".format(argPresetName), ["ok"]);
                        }, {
                            selectedIdx = frpresets.clump(2).detectIndex({|item| item[0] == argPresetName});
                        });

                        channelEQ.frdb = frpresets[(selectedIdx * 2) + 1].deepCopy;

                        //Find order of argPresetName
                        // frpresetsDict.keysDo{|key|
                        //     if (key == argPresetName, {
                        //         // i.postln;
                        //         selectedIdx = i;
                        //     },{i = i + 1;});
                        // };

                        ("selectedIdx: " + selectedIdx + "(" + argPresetName + ")").postln;
                        puMenu.value_(selectedIdx);
                    });
    ...
    init { |argChannelEQ|
        ...
        window = Window.new(
            if (channelEQ.target.isNil) {
                "ChannelEQ on bus % (% channels)".format(channelEQ.inBus, channelEQ.numChannels)
            } {
...
/*			'text', [[ 41.0, -5.5, 1 ], [ 250, 0, 1 ], [ 1710.0, 2.5, 1 ], [ 3500, 0, 1 ], [ 10556.0, -3.0, 1 ]],
			'moo', [[ 31.0, -3.5, 1 ], [ 220.0, 2.0, 0.5 ], [ 1377.0, 2.25, 1 ], [ 3500, 0, 1 ], [ 10556.0, -3.0, 1 ]],
			'string', [[ 59.0, 1.25, 1 ], [ 659.0, -1.25, 0.5 ], [ 1209.0, -5.0, 1 ], [ 3229.0, -2.5, 1 ], [ 10709.0, -1.75, 1 ]],
			'ending', [[ 41.0, 2.75, 1 ], [ 122.0, 5.25, 1 ], [ 311.0, 11.75, 0.1 ], [ 3500, 0, 1 ], [ 6000, 0, 1 ]]*/
        ];
